...
try {
    ...
    if (!$insertDataPrepare->affected_rows > 0){
        $email_sender_errors[] =  "There was a problem adding the user to the database.";
    }
...
if (isset($email_sender_errors) && !empty($email_sender_errors)) {
...
if(empty($email_sender_errors) && isset($_SESSION['email_has_been_sent'])){
    echo 
    " <ul class ='email_sent_message'>
        <p>Email sent</p>
